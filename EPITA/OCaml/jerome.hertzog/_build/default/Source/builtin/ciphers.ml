...
let encrypt_cesar k m b =
  if b>255 then
    invalid_arg "for extended ASCII table b should be set to 255"
  else
    let rec encrypt_cesar_rec k liste b = match liste with
        [] -> []
      | e::l when e<b-k -> let res = e+k in 
      res::encrypt_cesar_rec k l b
      | e::l -> encrypt_cesar_rec k l b
...
let decrypt_cesar k m b =
  if b>255 then
    ...
    let rec encrypt_cesar_rec k liste b = match liste with
        [] -> []
      | e::l when e>k && e<b -> (e-k)::encrypt_cesar_rec k l b
      | e::l -> encrypt_cesar_rec k l b
    in encrypt_cesar_rec k m b;;
